...
class TrainingOrchestrator:
    ...
    def load_training_function(self, config: TrainingConfig) -> TrainingFunction:
        ...
        if not hasattr(module, 'train'):
            raise ValueError(f"Training module {script_path} must have a 'train' function")
    ...
    def check_if_already_trained(self, config: TrainingConfig) -> bool:
        ...
        if hasattr(config, 'composite_run_key'):
            return self.run_exists_with_tag("composite_run_key", config.composite_run_key)
    ...
    def run_training(self, 
                    ...
                    force_retrain: bool = False) -> Optional[TrainingResult]:
        """
        ...
        if not force_retrain and self.check_if_already_trained(config):
            print(f"[SKIP] Model {config.model_type} already trained with current configuration")
            return None
    ...
    def train_multiple_configs(self, 
                             ...
                             y_train: pd.Series,
                             X_test: Optional[pd.DataFrame] = None,
                             y_test: Optional[pd.Series] = None) -> List[TrainingResult]:
        """
