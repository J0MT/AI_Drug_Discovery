...
def main():
    ...
    results = orchestrator.train_multiple_configs(configs_to_train, X_train, y_train, X_test, y_test)
    ...
    print(f"Models skipped: {len(configs) - len(results)} (idempotent - already trained)")
    
    if results:
        print("\nResults:")
        for result in results:
            print(f"  {result.model_info['model_type']}: RMSE={result.metrics.rmse:.4f}, R²={result.metrics.r2:.4f}")
